-- SIMPLE PACKAGE ID REFERENCE FIX WITH CORE COLUMNS ONLY
...
WITH best_my_packages AS (
  ...
  SELECT 
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY 
        (SELECT COUNT(*) FROM user_orders WHERE package_id::text = my_packages.id::text) DESC,  -- Most orders first
...
SELECT 
  ...
  (SELECT COUNT(*) FROM user_orders WHERE package_id::text = bmp.old_id::text) as order_count
...
-- STEP 5: Insert new package records with correct IDs (core columns only)
INSERT INTO my_packages (
  id, name, country_name, data_amount, sale_price, days, updated_at
)
SELECT 
  new_id, new_name, country_name, data_amount, new_price, new_days, NOW()
FROM temp_package_mappings
WHERE new_id::text NOT IN (SELECT id::text FROM my_packages);  -- Only insert if not already exists
...
FROM my_packages mp
INNER JOIN temp_package_mappings tm ON mp.id::text = tm.new_id::text;

-- STEP 7: Update user_orders to point to new package IDs
UPDATE user_orders
SET package_id = tm.new_id
FROM temp_package_mappings tm
WHERE user_orders.package_id::text = tm.old_id::text;
...
FROM user_orders uo
INNER JOIN temp_package_mappings tm ON uo.package_id::text = tm.new_id::text;

-- STEP 9: Delete old package records that are no longer referenced
DELETE FROM my_packages 
WHERE id::text IN (SELECT old_id::text FROM temp_package_mappings);
...
AND NOT EXISTS (
  SELECT 1 FROM user_orders WHERE package_id::text = my_packages.id::text
);
...
SELECT 
  ...
  SUM(CASE WHEN p.id IS NULL AND EXISTS(SELECT 1 FROM user_orders WHERE package_id::text = mp.id::text) THEN 1 ELSE 0 END) as orphaned_with_orders,
  ROUND(
...
SELECT 
  ...
  SUM(CASE WHEN EXISTS(SELECT 1 FROM user_orders WHERE package_id::text = my_packages.id::text) THEN 1 ELSE 0 END) as with_orders
FROM my_packages
...
SELECT 
  ...
  mp.data_amount,
  mp.name,
  CASE WHEN EXISTS(SELECT 1 FROM user_orders WHERE package_id::text = mp.id::text) THEN 'Has Orders' ELSE 'No Orders' END as order_status
FROM my_packages mp
WHERE NOT EXISTS (
  SELECT 1 FROM packages p 
