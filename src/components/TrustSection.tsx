  ...
  <section className="py-10 bg-white">
    ...
    >
        ...
        <div className="flex gap-6 mb-2">
          {badges.map((b, i) => (
            <div key={i} className="flex flex-col items-center">
              {b.svg}
              <span className="text-xs text-gray-600 mt-1">{b.label}</span>
            </div>
          ))}
        </div>
        <div className="text-gray-500 text-sm mt-2">Your payment is protected and encrypted.</div>
      </div>
      <div className="flex-1 flex flex-col items-center md:items-end gap-2">
        ...
        <div className="text-gray-400 text-xs mt-2">We partner with trusted payment providers.</div>
      </div>
    </motion.div>
  </section>
);
