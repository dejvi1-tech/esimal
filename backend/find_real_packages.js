...
async function findRealPackages() {
  console.log('🔍 Finding real Europe/US packages from Roamify...\n');
  ...
  try {
    ...
    // Look for any 3GB packages
    console.log('🔍 Looking for any 3GB packages:\n');
    const threeGbPackages = [];
    
    countries.forEach(country => {
      if (country.packages && Array.isArray(country.packages)) {
        country.packages.forEach(pkg => {
          if (pkg.dataAmount === 3072 || pkg.package.toLowerCase().includes('3gb')) {
            threeGbPackages.push({
              packageId: pkg.packageId,
              package: pkg.package,
              country: country.countryName,
              region: country.region,
    ...
    if (threeGbPackages.length > 0) {
      console.log(`Found ${threeGbPackages.length} 3GB packages:\n`);
      threeGbPackages.slice(0, 10).forEach((pkg, index) => {
        console.log(`${index + 1}. Package ID: ${pkg.packageId}`);
        console.log(`   Name: ${pkg.package}`);
        console.log(`   Country: ${pkg.country} (${pkg.region})`);
        console.log(`   Data: ${pkg.data}`);
