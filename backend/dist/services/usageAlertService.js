...
class UsageAlertService {
     ...
     */
    static async checkAndSendUsageAlerts() {
        try {
            logger_1.logger.info('🔔 Starting usage alert check...');
        ...
        `)
                ...
                .eq('status', 'paid');
            if (error) {
                logger_1.logger.error('Error fetching orders for usage alerts:', error);
                return;
            }
            logger_1.logger.info(`Found ${orders?.length || 0} active orders to check`);
            for (const order of orders || []) {
                await this.checkOrderUsage(order);
            }
            logger_1.logger.info('✅ Usage alert check completed');
        }
        catch (error) {
            logger_1.logger.error('Error in usage alert check:', error);
        }
    }
    /**
     * Check usage for a specific order and send alert if needed
     */
    static async checkOrderUsage(order) {
        try {
            const { iccid, user_email, user_name, data_amount } = order;
            if (!iccid || !user_email) {
                logger_1.logger.warn(`Skipping order ${order.id}: missing ICCID or email`);
                return;
            }
            // Get current usage from Roamify
            const { RoamifyService } = await Promise.resolve().then(() => __importStar(require('./roamifyService')));
            ...
            logger_1.logger.info(`Usage for ${iccid}: ${dataUsedGB}GB / ${dataLimitGB}GB (${usagePercentage.toFixed(1)}%)`);
            ...
            logger_1.logger.error(`Error checking usage for order ${order.id}:`, error);
        }
    }
    /**
     * Determine what type of alert should be sent
     */
    static determineAlertType(usagePercentage, dataRemainingGB) {
     ...
     */
    static async sendUsageAlert(order, usageData) {
        ...
        const { data: existingAlert } = await supabase_1.supabase
            .from('usage_alerts')
            .select('id')
            .eq('order_id', orderId)
            ...
            .single();
        if (existingAlert) {
            logger_1.logger.info(`Alert ${alertType} already sent for order ${orderId}`);
            return;
        }
        ...
        try {
            ...
            logger_1.logger.info(`✅ Usage alert sent: ${alertType} for ${user_email}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to send usage alert for ${user_email}:`, error);
        }
    }
    /**
     * Get email subject based on alert type
     */
    static getAlertSubject(alertType) {
        switch (alertType) {
            case '80_percent':
     ...
     */
    static getAlertEmailHtml(userName, usageData) {
        const { dataLimit, dataUsed, dataRemaining, usagePercentage, alertType } = usageData;
     ...
     */
    static getAlertMessage(alertType, usagePercentage) {
        switch (alertType) {
            case '80_percent':
