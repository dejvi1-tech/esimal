 ...
 */
const debugOrder = async (req, res, next) => {
    try {
      ...
      `)
        ...
        logger_1.logger.info(`Debug order request: ${orderId}`, {
            orderId,
            paymentIntentId: order.stripe_payment_intent_id,
            emailSent: order.email_sent,
            esimDelivered: order.esim_delivered,
        ...
        logger_1.logger.error('Error debugging order:', error);
        next(error);
    }
};
exports.debugOrder = debugOrder;
/**
 * Get package health overview
 */
const getPackageHealthOverview = async (req, res, next) => {
    try {
        logger_1.logger.info('Getting package health overview...');
        // Get basic stats
        const { count: myPackagesCount } = await supabase_1.supabase
            .from('my_packages')
            .select('*', { count: 'exact', head: true });
        const { count: packagesCount } = await supabase_1.supabase
            .from('packages')
            .select('*', { count: 'exact', head: true });
        // Get recent orders with potential issues
        const { data: recentFailedOrders, error: ordersError } = await supabase_1.supabase
            .from('orders')
        ...
        const cacheStats = (0, packageValidation_1.getValidationCacheStats)();
        // Sample package validation
        const { data: samplePackages } = await supabase_1.supabase
            .from('my_packages')
        ...
        if (samplePackages) {
            for (const pkg of samplePackages) {
                const result = await (0, packageValidation_1.validatePackage)(pkg.id);
        ...
        logger_1.logger.error('Error getting package health overview:', error);
        next(error);
    }
};
exports.getPackageHealthOverview = getPackageHealthOverview;
/**
 * Get sync status and history
 */
const getSyncStatus = async (req, res, next) => {
    try {
        logger_1.logger.info('Getting sync status...');
        // Check last sync from packages table
        const { data: lastSyncPackage } = await supabase_1.supabase
            .from('packages')
            .select('updated_at')
        ...
        const roamifyHealth = await roamifyService_1.RoamifyService.checkApiHealth();
        // Get recent package mappings with issues
        const { data: invalidMappings } = await supabase_1.supabase
            .from('my_packages')
 ...
 */
const triggerPackageValidation = async (req, res, next) => {
    try {
        ...
        logger_1.logger.info(`Validating ${packagesToValidate.length} packages...`);
        const validationResults = await (0, packageValidation_1.batchValidatePackages)(packagesToValidate);
        const summary = {
        ...
        logger_1.logger.error('Error triggering package validation:', error);
        next(error);
    }
};
exports.triggerPackageValidation = triggerPackageValidation;
/**
 * Get invalid packages that need review
 */
const getInvalidPackages = async (req, res, next) => {
    try {
        const { limit = 50 } = req.query;
        logger_1.logger.info('Getting invalid packages...');
        // Get packages without reseller_id
        const { data: missingResellerId } = await supabase_1.supabase
            .from('my_packages')
        ...
        const validRoamifyIds = new Set(roamifyPackages.map((p) => p.packageId));
        // Get packages with invalid reseller_id
        const { data: packagesWithResellerId } = await supabase_1.supabase
            .from('my_packages')
            .select('id, name, reseller_id, country_name, data_amount, days')
            .not('reseller_id', 'is', null)
            .limit(Number(limit) * 2); // Get more to filter
        const invalidResellerId = packagesWithResellerId?.filter(pkg => !validRoamifyIds.has(pkg.reseller_id)).slice(0, Number(limit)) || [];
        ...
        logger_1.logger.error('Error getting invalid packages:', error);
        next(error);
    }
};
exports.getInvalidPackages = getInvalidPackages;
/**
 * Trigger manual sync
 */
const triggerManualSync = async (req, res, next) => {
    try {
        ...
        logger_1.logger.info('Triggering manual sync...', options);
 ...
 */
const clearPackageValidationCache = async (req, res, next) => {
    try {
        const statsBefore = (0, packageValidation_1.getValidationCacheStats)();
        (0, packageValidation_1.clearValidationCache)();
        const statsAfter = (0, packageValidation_1.getValidationCacheStats)();
        ...
        logger_1.logger.error('Error clearing validation cache:', error);
        next(error);
    }
};
exports.clearPackageValidationCache = clearPackageValidationCache;
/**
 * Calculate health score based on validation results and failed orders
 */
function calculateHealthScore(validationSample, failedOrdersCount) {
...
const deduplicateMyPackages = async (req, res, next) => {
    try {
...
exports.deduplicateMyPackages = deduplicateMyPackages;
// Helper function to calculate my_package completeness score
function calculateMyPackageCompleteness(pkg) {
    let score = 0;
    if (pkg.name)
        score += 2;
    if (pkg.country_name)
    ...
    if (pkg.sale_price)
        score += 2;
    if (pkg.reseller_id)
        score += 2; // Higher weight for reseller_id
    if (pkg.region)
        score += 1;
    if (pkg.visible !== undefined)
        score += 1;
    if (pkg.show_on_frontend !== undefined)
        score += 1;
    return score;
}
