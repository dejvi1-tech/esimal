 ...
 */
export function validateMyPackageData(data: Partial<MyPackageData>): string[] {
  const errors: string[] = [];
 ...
 */
export function parseValidityToDays(validity: string | number): number | null {
  if (typeof validity === 'number') return validity > 0 ? validity : null;
