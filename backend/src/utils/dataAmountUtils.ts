...
export interface DataAmountResult {
  valueInGB: number;
  originalValue: string | number;
  unit: 'GB' | 'MB' | 'KB' | 'Unlimited';
}
 ...
 */
export function parseDataAmountToGB(input: string | number | null | undefined): number {
 ...
 */
export function formatDataAmountForDisplay(valueInGB: number): string {
 ...
 * - 51200 -> 50 GB
 */
export function fixIncorrectDataAmount(currentValue: number): number {
  const commonConversions: { [key: number]: number } = {
    1024: 1,
    3072: 3,
    5120: 5,
    10240: 10,
 ...
 */
export function extractDataAmountFromName(name: string): number {
  if (!name) return 0;
