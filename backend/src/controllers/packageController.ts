...
export const syncRoamifyPackages = async (
  ...
  try {
    ...
    // Fetch all packages from Roamify API with pagination
    ...
    while (true) {
      ...
      try {
        ...
        // Debug: Log the raw response structure for first page
        if (page === 1) {
          console.log('=== RAW ROAMIFY API RESPONSE (Page 1) ===');
        ...
        console.error(`❌ Failed to fetch page ${page} from Roamify:`, error);
        throw error;
      }
    }
